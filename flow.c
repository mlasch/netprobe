...
static bool metacmp(struct pkt_meta *a, struct pkt_meta *b) {
    ...
    if (memcmp(&a->ip_addr, &b->ip_addr, sizeof(struct in6_addr)) != 0)
        return false;
    if (a->dir != b->dir)
        return false;
    if (a->proto != b->proto)
...
void flow_to_post(struct flow *f_ptr, char *buf) {
    ...
    sprintf(buf, ",mac=%02x:%02x:%02x:%02x:%02x:%02x,dir=%s,ip=%s,proto=%s bytes=%d,packets=%d", ptr->meta.mac[0],
            ptr->meta.mac[1], ptr->meta.mac[2], ptr->meta.mac[3], ptr->meta.mac[4], ptr->meta.mac[5],
            dir_to_str(ptr->meta.dir, dir_buf), inet_ntop(AF_INET6, &ptr->meta.ip_addr, ip_buf, INET6_ADDRSTRLEN),
